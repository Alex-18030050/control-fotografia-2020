...
namespace Graduaciones_Karyme.Forms
{
    public partial class frm_escuelas : Form
    {
        ...
        private void Max()
        {
            ...
            conexion.Open();
            string query = "select max(IA_Id)+1 as ultimo from INSTITUCIONES_ACADEMICAS where IA_Estatus = 1";
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            ...
            conexion = new SqlConnection(obj_conexion.Con());
            conexion.Open();
            string query = "UPDATE INSTITUCIONES_ACADEMICAS set IA_Estatus = 0 where IA_Id = @IA_Id";
            SqlCommand comando = new SqlCommand(query, conexion);
            comando.Parameters.Clear();
            comando.Parameters.AddWithValue("@IA_Id", iaSchool);
            comando.ExecuteNonQuery();
            MessageBox.Show("Escuela eliminada con exito", "Operacion Completada", MessageBoxButtons.OK, MessageBoxIcon.Information);
            string accion = "Dio de baja una escuela.";
            Clases.cl_globales hecho = new Clases.cl_globales();
            hecho.auditoria(username3, accion);
            LlenarEscuelas();
            Clean();
        ...
        private void AgregarNuevo() // ToDo: Hacer with only empleados 
        {
            ...
            conexion = new SqlConnection(obj_conexion.Con());
            conexion.Open();
            string query = "INSERT into INSTITUCIONES_ACADEMICAS VALUES(@IA_Id, @IA_IdLocalidad, @IA_Nombre, @IA_Telefono, @IA_Email, @IA_ReNom, @IA_IdColonia,1)";
            SqlCommand comando = new SqlCommand(query, conexion);
            comando.Parameters.Clear();
            comando.Parameters.AddWithValue("@IA_Id", iaSchool); 
        ...
        private void Actualizar() // HACER UN UPDATE
        {
            obj_conexion = new Clases.Conexion();
            conexion = new SqlConnection(obj_conexion.Con());
            conexion.Open();
